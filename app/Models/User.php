...
class User extends Authenticatable
{
    ...
    {
        $basic = $this->salary;
        if ($this->salaryChanges != '') {
            foreach ($this->salaryChanges()->orderBy('id','asc')->get() as $salary) {
                if (strtotime($year.'-'.$month.'-25') >= strtotime($salary->fromYear.'-'.$salary->fromMonth.'-1')) {
                    $basic = $salary->Salary;
                }
            }
        }
        if ($month == date('m',strtotime($this->employment_date)) && $year == date('Y',strtotime($this->employment_date))) {
            $d1 = date_create($this->employment_date); //current month/year
            $d2 = date_create($d1->format('Y-m-t')); //get last date of the month

            $workingDays = date_diff($d1,$d2);
            $workingDays = $workingDays->days+1;
            $monthDaysCount=cal_days_in_month(CAL_GREGORIAN,$month,$year);

            $daysPercentage = $workingDays/$monthDaysCount;

            $basic = $basic*$daysPercentage;
        }
        return round($basic);
    }
    ...
    {
        return $this->hasMany(EmployeeSalaryChanges::class,'EmployeeID');
    }
