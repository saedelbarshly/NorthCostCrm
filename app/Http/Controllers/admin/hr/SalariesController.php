...
class SalariesController extends Controller
{
    ...
    public function index()
    {
        ...
        foreach ($users as $key => $value) {
            if (strtotime($value['employment_date']) <= strtotime(date($month.'/28/'.$year))) {
                if ($value['status'] != 'Archive') {
                    $usersList[] = $value;
                } else {
                    if (strtotime($value['unemployment_date']) <= strtotime(date($month.'/28/'.$year))) {
                        $usersList[] = $value;
                    }
                }
    ...
    public function payOutSalary(Request $request, $user_id)
    {
        ...
        $data = $request->except(['_token','SafeID']);
        $thisUser = User::find($user_id);
        $data['forMonth'] = $month;
        $data['forYear'] = $year;
        $data['month'] = $month; //date('m',strtotime($request['Date']));
        $data['year'] = $year; //date('Y',strtotime($request['Date']));
        $data['UID'] = auth()->user()->id;
        $data['branch_id'] = $thisUser->branch_id;
        $data['EmployeeID'] = $user_id;
        $data['safe_id'] = $request['SafeID'];
