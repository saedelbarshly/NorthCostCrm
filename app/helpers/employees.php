...
function salariesStats($branch = 'all', $month = null, $year = null)
{
    ...
    foreach ($users as $user) {
        if (strtotime($user['employment_date']) <= strtotime(date($month.'/28/'.$year))) {
            if (strtotime($user['employment_date']) <= strtotime(date($month.'/28/'.$year))) {
                if ($user['status'] != 'Archive') {
                    ...
                    if (strtotime($user['unemployment_date']) <= strtotime(date($month.'/28/'.$year))) {
                        $list['basic'] += $user->monthSalary($month, $year)['basic'];
                        $list['plus'] += $user->monthSalary($month, $year)['plus'];
                        $list['minus'] += $user->monthSalary($month, $year)['minus'];
                        $list['delivered'] += $user->monthSalary($month, $year)['delivered'];
            ...
            //     $list['net'] += $user->monthSalary($month, $year)['net'];
            // } else {
            //     if (strtotime($user['unemployment_date']) >= strtotime(date($month.'/01/'.$year))
            //         && strtotime($user['unemployment_date']) <= strtotime(date($month.'/28/'.$year))) {
            //         $list['basic'] += $user->monthSalary($month, $year)['basic'];
            //         $list['plus'] += $user->monthSalary($month, $year)['plus'];
            //         $list['minus'] += $user->monthSalary($month, $year)['minus'];
            //         $list['delivered'] += $user->monthSalary($month, $year)['delivered'];
...
function attendancePermissions($lang)
{
    $list = [
        'ar' => [
            'late' => 'تأخير حضور',
            'early' => 'انصراف مبكر',
            'temporaryDismissal' => 'انصراف مؤقت',
            'overTime' => 'تكليف ساعات إضافية'
        ],
        'en' => [
            'late' => 'تأخير حضور',
            'early' => 'انصراف مبكر',
