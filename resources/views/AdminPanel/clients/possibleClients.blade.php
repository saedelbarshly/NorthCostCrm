...
@section('content')
    ...
    $status = '';
    $position = '';
    if ($active == 'possibleClients') {
        $position = 'call_center';
    } elseif ($active == 'receptionClients') {
        $position = 'reception';
    } elseif ($active == 'clientsContracts') {
        $position = 'contract';
    } elseif ($active == 'clientsArchive') {
        $position = 'archive';
    } else {
        $position = 'sales';
    }
                    ...
                    <div class="card text-center">
                        <div class="card-body">
                            <a href="{{route('admin.possibleClients',$params)}}">
                                <h2 class="fw-bolder">
                                    {{clientsPageStats($month, $year, $employee, $source, $Name, $cellphone, $identity, 'call_center')[$index]}}
                                </h2>
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                        ...
                        @if (userCan('client_asignment'))
                            ...
                            <small>
                                <div class="form-check">
                                    ...
                                    <label class="form-check-label" for="selectAll"> {{ trans('common.Select All') }}
                                    </label>
                                </div>
                            </small>
                        @endif
                    ...
                    @if (userCan('client_asignment'))
                        <a href="javascript:;" data-bs-target="#transferClients" data-bs-toggle="modal"
                        ...
                        <tbody>
                            @forelse($clients as $client)
                                <tr class="row_{{ $client->id }}" style="border-top: solid #666;">
                                    @include('AdminPanel.clients.clients-table-row', [
                                        'client_row' => $client,
                                    ])
                                </tr>
...
@section('page_buttons')
    ...
    @if (userCan('clients_create'))
        ...
        @include('AdminPanel.clients.create', ['position' => 'call_center'])
    @endif
